...
function dgx_donate_shortcode( $atts ) {
	...
	if ( $show_thanks ) {
		...
		if ( $sd4_mode != false ) {
			...
			$output .= "Please update using the [seamless-donations] shortcode. ";
			$output .= "The shortcode is the chunk of text on your WordPress page ";
			$output .= "that generates the donation form. If you're getting this ";
			$output .= "message, it means you updated from a very out-of-date plugin ";
			$output .= "and need to change your page to replace [dgx-donate] with ";
			$output .= "[seamless-donations]. See <a href=\"https://youtu.be/SWm6GivlJi0\">this video</a> ";
			$output .= "for more details. Visit <a href=\"http://zatzlabs.com\">ZATZLabs.com</a> for support.</p>";
			$output .= "</div>";
		} else {
...
function dgx_donate_send_donation_notification( $donationID ) {
	...
	$body
	           = __( 'A donation has been received.  Here are some details about the donation.',
	                 'seamless-donations' ) . "\n";
	$body .= "\n";
