	...
	<div class="classic-event-wrapper">
		<div class="row">
			<div class="grid-1-3">
				...
				<?php endif ?>
				<h3 class="event-title"><a href="<?php echo tribe_get_event_link( $event ) ?>" title="<?php esc_attr_e( 'Read More', 'church-event' ) ?>"><?php echo $event->post_title // xss ok ?></a></h3>
			</div>
			<div class="grid-2-3">
				...
				<div class="split"></div>
				<a href="<?php echo tribe_get_event_link( $event ) ?>" title="<?php esc_attr( $read_more_text ) ?>" class="vamtam-button button accent1 hover-accent1"><span class="btext"><?php echo $read_more_text // xss ok ?></span></a>
				<?php if ( ! empty( $view_all_text ) && ! empty( $view_all_link ) ): ?>
					<span class="view-all-wrapper">
						<?php _e( 'or', 'church-event' ); ?>
