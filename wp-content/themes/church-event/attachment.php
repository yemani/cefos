...
$title = __('Attachment', 'church-event');
get_header();
?>
	...
	<div class="row page-wrapper">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(WpvTemplates::get_layout()); ?>>
			...
			<div class="page-content">
				...
				<div class="entry-caption">
					...
					<?php
						if ( wp_attachment_is_image() ) {
							...
							printf( __( 'Original size is %s pixels', 'church-event'),
								sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
									wp_get_attachment_url(),
									esc_attr( __('Link to full-size image', 'church-event') ),
									$metadata['width'],
									$metadata['height']
								)
							);
				...
				<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'church-event' ) ); ?>
