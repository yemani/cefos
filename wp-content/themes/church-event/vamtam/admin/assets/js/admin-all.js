	...
	$.fn.wpvIconsSelector = function(atts) {
		...
		(function() {
			...
			var scrollIcons = function(self) {
				self.each(function() {
					var icons_wrapper = $(this).find('.icons-wrapper');
					var checked       = $(this).find(':checked + label');

					if ( icons_wrapper && checked && checked.offset() ) {
						icons_wrapper.scrollTop(0); // reset the inital position
						icons_wrapper.scrollTop(checked.offset().top - icons_wrapper.offset().top);
					}
				});
			};
