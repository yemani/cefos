...
class WpvConfigGenerator {
	...
	public static function print_templates() {
		$templates = apply_filters('wpv-admin-backbone-template-files', array(
        ...
        switch($type){
			...
			case 'tribe_events_category':
				...
				foreach($entries as $key => $entry)
					$config[$entry->slug] = $entry->name;
			break;
			case 'wpv_sermons_category':
				$entries = get_terms('wpv_sermons_category', 'orderby=name&hide_empty=0');
				foreach($entries as $key => $entry)
					$config[$entry->slug] = $entry->name;
			break;
			case 'testimonials':
