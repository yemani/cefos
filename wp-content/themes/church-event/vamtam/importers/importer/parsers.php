		...
		if ( $fp ) {
			while ( ! $this->feof( $fp ) ) {
				...
				foreach ( $multiline_tags as $tag => $handler ) {
					// if the opening and closing tags are on the same lien $multiline_content may be empty
					// we need to use $importline in this case
					...
					if ( false !== strpos( $importline, "</$tag>" ) ) {
						...
						$this->{$handler[0]}[] = call_user_func( $handler[1], $opening_tag_found ? $importline : $multiline_content );
