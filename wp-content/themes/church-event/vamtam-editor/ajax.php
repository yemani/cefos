...
class WPV_Editor_AJAX {
	...
	private function markup_handler( $id, $attributes=null, $content='' ) {
		...
		?>
			<div id="<?php echo esc_attr( $this->get_uniqid( $id ) ) ?>" class="wpv_ed_column wpv_sortable inner-sortable column column-<?php echo esc_attr( preg_replace( '/column-(\d)(\d)/', '$1-$2', $id ) ) ?> expandable expanded" data-basename="column">
				<?php echo $this->getControls( 'size name add edit clone delete handle', preg_replace( '/column-(\d)(\d)/', '$1/$2', $id ), __( 'Column', 'church-event' ), $attributes ) // xss ok ?>
		...
		<?php else : ?>
			...
			<?php
				...
				$accepting = isset( $wpv_sc[$id]["accepting"] ) && $wpv_sc[$id]["accepting"] ? 'inner-sortable expanded' : '';
			...
			<div id="<?php echo esc_attr( $this->get_uniqid( $id ) ) ?>" class="wpved-<?php echo esc_attr( "$id $accepting $class" ) ?> wpv_sortable column <?php echo esc_attr( "$size_class $expandable" ) ?>" <?php echo $this->getCallbacks( $id ) ?> data-basename="<?php echo esc_attr( $id ) ?>">
				<?php echo $this->getControls( $controls, $size, $wpv_sc[$id]['name'], $attributes ) // xss ok ?>
	...
	private function getControls( $controls, $size_str, $name='', $atts = array() ) {
		...
		$controls_html = array(
			...
			'size' => "<div class='column-size-wrapper'>
							<div class='column-increase-decrease'><a class='column-increase icon-plus' href='#' title='" . esc_attr__( 'Increase width', 'church-event' ) . "'></a> <a class='column-decrease icon-minus' href='#' title='" . esc_attr__( 'Decrease width', 'church-event' ) . "'></a></div>
							<span class='column-size'>$size_str</span>
						</div>",
			// 'add' => " <a class='column-add icon-plus' href='#' title='".__( 'Insert element', 'church-event' )."'></a> ",
			'clone' => " <a class='column-clone icon-copy' href='#' title='" . esc_attr__( 'Clone', 'church-event' )."'></a> ",
			'edit' => ' <a class="column-edit icon-edit" href="#" title="' . esc_attr__( 'Edit shortcode properties', 'church-event' ).'"></a>',
			'delete' => ' <a class="column-remove icon-remove" href="#" title="' . esc_attr__( 'Remove shortcode', 'church-event' ).'"></a>',
			'handle' => '<div class="handlediv" title="' . esc_attr__( 'Click to toggle', 'church-event' ).'"><br /></div>',
		);
	...
	private function get_special_markup( $id, $content ) {
		...
		switch ( $id ) {
				...
				foreach ( $sub_contents as $i=>$text ) : ?>
					...
					<div>
						<h3 class="title-wrapper clearfix">
							<a class="accordion-title"><?php echo $params[$i]['title'] // xss ok ?></a>
							<a class="accordion-remove icon-remove" title="<?php esc_attr_e( 'Remove', 'church-event' ) ?>"></a>
							<a class="accordion-clone icon-copy" title="<?php esc_attr_e( 'Clone', 'church-event' ) ?>"></a>
							<a class="accordion-background-selector" data-background-image="<?php if ( isset( $params[$i]['background_image'] ) ) echo esc_attr( $params[$i]['background_image'] ) ?>" title="<?php esc_attr_e( 'Change Pane Background', 'church-event' ) ?>"><?php wpv_icon( 'image' ) ?></a>
						</h3>
		 ...
		 'icon' => '',
				...
				?>
					<li>
						<a href="#tabs-<?php echo esc_attr( $suffix . $i ) ?>" class="tab-title"><?php echo $p['title'] // xss ok ?></a>
						<a class="tab-remove icon-remove" title="<?php esc_attr_e( 'Remove', 'church-event' ) ?>"></a>
						<a class="tab-clone icon-copy" title="<?php esc_attr_e( 'Clone', 'church-event' ) ?>"></a>
						<a class="wpv-icon-selector-trigger tab-icon-selector vamtam-icon <?php echo esc_attr( wpv_get_icon_type( $p['icon'] ) ) ?> <?php if ( empty( $p['icon'] ) ) echo 'no-icon' ?>" data-icon-name="<?php echo esc_attr( $p['icon'] ) ?>" title="<?php esc_attr_e( 'Change Icon', 'church-event' ) ?>"><?php wpv_icon( $p['icon'] ) ?></a>
					</li>
				<?php endforeach;
				...
				foreach ( $sub_contents as $i=>$text ) : ?>
					<div id="tabs-<?php echo $suffix.$i ?>" class="clearfix inner-sortable"><?php echo $this->do_parse( $text ) // xss ok ?></div>
				<?php endforeach;
			...
			case 'services_expandable':
				echo "<textarea class='inner-content'>$content</textarea>"; // xss ok
			break;
