...
(function($, undefined) {
	...
	$(function() {
		...
		if (ED.container.length === 1) {
			...
			$('#submitdiv').bind('mouseneter.vamtameditor', function() {
				ED.saveHTML();
			});
		}
	});
		...
		generate_shortcodes: function(root, level) {
			...
			columns.each(function() {
				...
				if (element.hasClass('wpv_ed_column')) {
					...
					element.find('> .controls .column-size').length ||
					(column_title && column_title.length > 1) ||
					column_divider ||
					(column_animation && column_animation !== 'none')
				) {
