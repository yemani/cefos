...
class WPV_Editor {
	...
	public function third_party_shortcodes( $available_shortcodes ) {
		...
		if ( is_plugin_active( 'vamtam-sermons/vamtam-sermons.php' ) )
			$available_shortcodes['wpv_sermons'] = 'layouts';
	...
	public function admin_init() {
		...
		$post_types = array( 'post', 'page', 'portfolio', 'wpv_sermon', 'tribe_events' );
		foreach ( $post_types as $type ) {
			add_meta_box( 'wpv_visual_editor', __( 'Visual Editor', 'church-event' ), array( &$this, 'editor' ), $type, 'advanced', 'low' );
		}
	...
	public function enqueues() {
		...
		wp_localize_script( 'wpv-editor', 'WPVED_LANG', array(
				'empty_notice' => __( 'Please drag  any element you want here.', 'church-event' ),
			) );
	}
							...
							<?php if ( isset( $wpv_sc[$slug]['desc'] ) ): ?>
								<div class="description">
									...
									<div>
										...
										<footer><a href="<?php echo esc_attr(admin_url( 'admin.php?page=wpv_help' )) ?>" title="<?php _e( 'Read more in our documentation', 'church-event' ) ?>"><?php _e( 'Read more in our documentation', 'church-event' ) ?></a></footer>
									</div>
								</div>
							<?php endif ?>
						</li>
